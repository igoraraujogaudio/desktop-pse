...
export function TrocaModal({ isOpen, onClose, onConfirm, onReadyForValidation, solicitacao, inventoryItem }: TrocaModalProps) {
    ...
    const [gerarDesconto, setGerarDesconto] = useState(false);
    ...
    // Automatically set gerarDesconto to true when critical condition is selected
    if (isCritical && !gerarDesconto) {
        setGerarDesconto(true);
    }
    ...
    const handleSubmit = async () => {
        ...
        // For critical conditions, discount order is mandatory
        if (isCritical && !gerarDesconto) {
            alert('Ordem de desconto é obrigatória para itens danificados ou perdidos.');
            return;
        }
